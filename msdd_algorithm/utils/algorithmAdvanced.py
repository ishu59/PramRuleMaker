...
class RuleGenBase:
    ...
    def __init__(self, precursor: Union[List, Tuple, str], successor: Union[List, Tuple, str],
        ...
        if self.successor_attribute is None:
            self.successor_attribute = ['attribute_' + str(i) for i in range(len(successor))]
...
def msdd_algorithm_simple(sequence_data=None, precursor_width=None, successor_width=None, lagtime_between=None,
    ...
    '''
    #  Todo: create combinations of successor and iterate over them
    # create a new dictionary with prec all combination and successor all combination
    rule_gen_list: List[RuleGenBase] = []
    ...
    all_successor_combination_set = set()
    ...
    all_combination_p_s_dict: Dict[Tuple, int] = {}
    ...
    for p_s_key, count_val in p_s_token_dict.items():
        ...
        local_successor_set = set()
        for i in range(1, len(successor_l) + 1):
            items = combinations(successor_l, i)
            items = list(items)
            for item in items:
                all_successor_combination_set.add(item)
                local_successor_set.add(item)

        local_precursor_set = frozenset(local_precursor_set)
        local_successor_set = frozenset(local_successor_set)

        new_key_p_s = tuple([local_precursor_set, local_successor_set])
    ...
    for unique_prec_key in all_precursor_combination_set:
        for unique_succ_key in all_successor_combination_set:
            for combined_p_s_key, count_val in all_combination_p_s_dict.items():
                precursor_m, successor_m = combined_p_s_key
                if unique_prec_key in precursor_m and unique_succ_key in successor_m:
                    new_ind_p_s_key = tuple([unique_prec_key, unique_succ_key])
    ...
    # pprint(ind_combination_precursors_successor_count_dict)
    # pprint(ind_precursor_count_dict)
    # pprint(ind_successor_count_dict)
    ...
    pprint(rule_json_value)
    return rule_json_value


def test_msdd_simple():
    rule_dict = msdd_algorithm_simple(data_type_vertical=True)
    print(rule_dict)
